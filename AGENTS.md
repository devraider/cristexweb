...
- Python exists in offline contract tests and in the three exact-scope Ansible action plugins that enforce canonical Namespace, Infisical object-closure, and proxy Secret mutations at the mutation boundary. These reviewed plugins are the focused exception permitted by the host automation standard; no general-purpose operational Python or infrastructure collector exists.
