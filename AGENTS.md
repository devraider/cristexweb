...
- Python exists in offline contract tests and in five exact-scope Ansible action plugins: four enforce canonical Namespace, Infisical/Argo object-closure, and proxy Secret mutations at the mutation boundary, while one performs no-log cryptographic validation of the exact Argo Secret contract. These reviewed plugins are the focused exception permitted by the host automation standard; no general-purpose operational Python or infrastructure collector exists.
