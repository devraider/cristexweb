...
- Python exists in offline contract tests and in thirty-nine exact-scope Ansible action plugins, one exact-scope backup strategy plugin, and one focused Ansible library module. These reviewed focused guards cover canonical Namespace, Infisical/Argo/database, CristexHub DEV/PROD, stateful, identity, route, registration, provisioning, NetworkPolicy, and backup task-selection boundaries. They are focused exceptions permitted by the host automation standard; no general-purpose operational Python or infrastructure collector exists.
