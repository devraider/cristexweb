...
class ReactiveResumeDevArgoRegistrationContractTests(unittest.TestCase):
    def test_exact_registration_closure(self):
        ...
        self.assertEqual('5a63a9c743b54dd2cebaac190e370871b929c428', app['spec']['source']['targetRevision'])
    ...
    def test_hash_bound_guarded_entrypoint_and_runbook(self):
        ...
        self.assertIn('b262ddb6834eb9d14d0eb279bb1a1c8686df83fedea56dc51d01fddc2281a3ac', defaults)
        ...
        self.assertIn('Require the exact completed migration Job source and live immutable spec', tasks)
        self.assertIn('migration_job_contract', tasks)
        self.assertIn('migration_job_source_sha256', tasks)
        self.assertIn('migration_source.spec.template.spec.containers[0].env', tasks)
        self.assertIn('migration_source.spec.template.spec.containers[0].image', tasks)
        self.assertIn('migration_generated_label_keys', tasks)
        self.assertIn('EXPECTED_MIGRATION_SOURCE_SHA256', plugin)
        ...
        self.assertIn('reactive_resume_dev_argocd_registration_internal_alignment_contract', tasks)
        self.assertIn('reactive_resume_dev_argocd_registration_internal_alignment_sources', tasks)
        self.assertIn("item.content | b64decode | hash('sha256') == item.item.sha256", tasks)
        self.assertIn("stdin: '{{ item.content | b64decode }}'", tasks)
        self.assertIn('reactive_resume_dev_argocd_registration_internal_desired_diffs.results | length == 7', tasks)
        self.assertIn('reactive_resume_dev_argocd_registration_internal_destination_diffs.results | length == 4', tasks)
        ...
        self.assertIn('LIVE / SYNCED / HEALTHY', runbook)
        self.assertIn('Synced/Healthy', runbook)
        ...
        self.assertIn('registration and handoff now pass', runbook)
        self.assertIn('apply and handoff evidence', runbook)
