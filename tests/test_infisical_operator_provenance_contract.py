...
class InfisicalOperatorCandidateProvenanceContractTests(unittest.TestCase):
    ...
    def test_namespace_rbac_secret_zero_and_compatibility_blockers_are_explicit(self) -> None:
        ...
        for required in (
            ...
            "exact present-only source and a distinct bounded Ansible wrapper exist for `shared-services`",
            "check and separately approved first apply/idempotence passed",
            "final `changed=0`",
            "all Operator checkpoints remain separately approved and **NOT RUN**",
            ...
            "Runtime approvals",
            "foundation Namespace check, first apply, and idempotence are complete",
