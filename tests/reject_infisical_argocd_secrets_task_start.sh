...
infisical_argocd_secrets_bootstrap_approved: true
infisical_argocd_secrets_bootstrap_internal_preflight_binding:
  attestation_sha256: $attestation_sha256
  object_count: 11
  identity_set_sha256: bbfddcc3da0de1de6da0632fb51fb7a364960f7080e3eeca4094a267b27e1698
  prestate_count: 11
  admission_count: 6
  rbac_count: 2
  source_count: 3
  ...
  crd_count: 6
infisical_argocd_secrets_bootstrap_internal_rbac_objects:
YAML
for manifest in \
  "$repository_root/ansible/files/components/infisical-argocd-secrets/rbac/argocd-secret-writer-role.yaml" \
  "$repository_root/ansible/files/components/infisical-argocd-secrets/rbac/argocd-secret-writer-rolebinding.yaml"; do
...
set +e
CRISTEXWEB_INFISICAL_ARGOCD_SECRETS_BOOTSTRAP_ENTRYPOINT=v1 \
CRISTEXWEB_INFISICAL_ARGOCD_SECRETS_BOOTSTRAP_TOKEN="$attestation_token" \
CRISTEXWEB_INFISICAL_ARGOCD_SECRETS_BOOTSTRAP_ATTESTATION_FILE="$attestation_file" \
  "$repository_root/.venv/bin/ansible-playbook" -i "$temporary_directory/inventory.yml" \
  playbooks/bootstrap_infisical_argocd_secrets.yml --check --diff --limit crtxweb \
  --extra-vars "@$injected_variables" --start-at-task \
  'infisical_argocd_secrets_bootstrap : Reconcile exact Infisical Argo CD Secret seam RBAC after admission' >"$output_file" 2>&1
readonly status=$?
set -e
[[ $status -ne 0 ]]
grep -Eq 'TASK_SELECTION_GUARD|ENTRYPOINT_GUARD' "$output_file"
! grep -Fq 'INTERNAL_VARIABLE_GUARD' "$output_file"
printf '%s\n' 'PASS: Infisical Argo CD Secret seam task-start and injected-binding bypass is rejected'
