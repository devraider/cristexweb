...
class RuntimeSeamTests(unittest.TestCase):
 ...
 def test_composition_entrypoint_is_guarded_and_value_safe(self):
  ...
  self.assertIn('PRIVATE_CA_BUNDLE',t); self.assertIn('CODE_RUNNER_AUTH_TOKEN',t); self.assertIn('BROWSERLESS_TOKEN',t); self.assertIn('DOCKER_CONFIG_JSON',t); self.assertIn("openssl rand -hex 32 | tr -d '\\n'",t); self.assertIn('atomic',POLICY.read_text())
