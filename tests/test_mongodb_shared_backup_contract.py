...
class MongoDBSharedBackupContractTests(unittest.TestCase):
    ...
    def test_test_and_restore_persist_exact_sanitized_acceptance_receipts(self) -> None:
        for value in (
            ...
            "_SYSTEMD_INVOCATION_ID,MESSAGE",
            "mongodb_shared_backup_test_journal.stdout_lines | map('from_json') | list",
            "mongodb_shared_backup_test_receipts[0]._SYSTEMD_INVOCATION_ID is match('^[0-9a-f]{32}$')",
