...
- `github_actions_repository_permissions.reactive_resume_mirror`
  ...
  - no GitHub-owned, verified, or pattern-allowed actions
...
2. perform read-only duplicate/repository inventory and import an existing matching
   repository instead of creating a duplicate;
3. back up and rehearse recovery of `github.tfstate`;
4. review a plan containing only repository metadata creation or convergence;
5. reject any replacement, destroy, source-file, secret, webhook, deploy-key,
   unexpected Actions, package, collaborator, team, or Cloudflare address; require
   the exact Actions-disabled resource and reject any permissive Actions setting;
6. apply only after explicit GitHub provider and repository approval, then require a
   second no-op plan and private repository post-state verification.
