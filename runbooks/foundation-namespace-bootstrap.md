...
The exact source for `platform-secrets` and `platform-identity` and a dedicated
bounded Ansible bootstrap are implemented. The source is deployable, but every
runtime checkpoint is **NOT RUN**. This record authorizes no discovery, wrapper
check, apply, idempotence run, Secret operation, workload, route, or cluster contact.
...
A live run may only create or reconcile the two exact Namespaces with state present.
...
2. Review the complete check/diff result and require that only `platform-secrets` and `platform-identity` are predicted.
...
Until those checkpoints pass, both Namespace runtime states remain **NOT RUN** and no
new cluster object is claimed.
