...
schedule, retention, local staging path, or executable job. No backup or restore has
run.
...
identity, root folder identity, credentials, retention, and recovery procedure are
selected. Use `rclone copy`; never `rclone sync`. Destructive mirror semantics are
forbidden.
...
staging path and capacity, schedule, local/off-node retention, credential recovery,
encryption-key recovery, and RPO/RTO remain unselected or unproved.
