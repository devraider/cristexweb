...
4. **Shared operator Namespace:** exact present-only source and a distinct bounded
   ...
   apply/idempotence passed, with final `changed=0`; all Operator checkpoints remain
   separately approved and **NOT RUN**. The completed
...
12. **Runtime approvals:** the foundation Namespace check, first apply, and
    idempotence are complete; separately approve operator
