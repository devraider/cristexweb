...
is a planned private DEV MVP in `cristexhub-dev`; it is not installed. This revision
adds only a value-free blocker inventory and does not add a deployable manifest,
...
- **Dedicated Infisical lane:** the existing broad 11-target database seam,
  ...
  existing match-condition skip behavior, live CEL type-check warnings, exact
  allowlist expansion, complete foreign-object inventory, and truth-table tests. A
...
- **Dedicated PostgreSQL lane:** the current all-consumer provisioning wrapper and
  ...
  `NOBYPASSRLS`; revoke PUBLIC connect/schema creation, grant only the exact own
  database privileges, and run every DEV↔PROD/CristexHub/Keycloak/role-creation
...
- **OIDC:** new DEV identity targets the successor `cristexhub-dev` realm and issuer;
  that realm and its Reactive Resume client are not live. The retained `cristexhub`
  realm remains PROD-compatible and read-only during this transition. Do not mutate
  either client from this policy. Before runtime, bind issuer discovery/JWKS, the
  exact v5 custom callback, web origin, post-logout behavior,
  PKCE S256, scopes, audience, claims, group authorization, logout/account-linking,
  and positive/negative wrong-issuer, wrong-audience, missing/ambiguous-group,
  expired-token, and cross-environment tests. PROD gets a separate client only after
  DEV acceptance.
- **Object storage:** PostgreSQL is not the whole application state. Uploaded
  resumes, images, PDFs, cover letters, and Agent attachments require a selected
  private object-storage backend, separate DEV/PROD bucket or prefix, encryption,
  versioning/immutable manifests, URL continuity, checksum/readback, and a
  database/object-consistent encrypted backup and isolated restore. Local ephemeral
  container storage is forbidden.
- **Redis and AI:** the Agent/AI feature is disabled until explicitly selected.
  Redis is forbidden until that decision; if selected, it needs separate private
  authenticated DEV/PROD scopes, resource/eviction policy, recovery semantics, and
  object storage for attachments. Redis active streams are transient/non-authoritative
  unless a reviewed release proves otherwise.
- **Application keys:** exact key names remain unselected until one upstream release
  ...
  and conditional `ENCRYPTION_SECRET`; they require dedicated Infisical custody.
  `ENCRYPTION_SECRET` is conditional on encrypted
  provider/Agent state. Key names, per-environment paths, independent custody,
  ...
  evidence.
- **Migrations and recovery:** automatic migrations on every application startup
  are forbidden. A dedicated migration actor, lock, pre-migration backup,
  forward-compatible expand/contract policy, and schema rollback evidence are
...
database principals, migrations, object-storage paths, Redis scopes, or backup
...
PROD is represented only as a promotion template for `cristexhub-prod`,
`reactive-resume-prod`, `reactive_resume_prod`, and separate credential/backup
scopes. It has no generated manifests, runtime objects, Secret values, selected
...
## No runtime operation in this revision

No Deployment, StatefulSet, Service, PVC, Secret, Infisical CR, Database object,
Ingress, Argo Application, route, image pull, registry lookup, GitHub runner,
host, Kubernetes API, Infisical API, database, or provider operation was performed.
