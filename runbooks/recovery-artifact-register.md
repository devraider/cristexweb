...
| cloudflared component artifact | Human-selected version, accepted publisher/provenance trust, immutable architecture-specific image digest, image assurance review, tested hardening/health behavior, soak decision, and independent off-node availability; the source-only [candidate provenance](cloudflared-candidate-provenance.md) is research evidence, not selection or recovery proof | `<artifact/decision evidence record ID>` | CANDIDATE EVIDENCE ONLY — STOP |
| Infisical Operator component artifact | Human-selected coherent chart/source/image set, accepted signer/build trust, immutable architecture-specific image digest, target compatibility, scoped-RBAC decision, soak decision, and independent off-node availability; the source-only [candidate provenance](infisical-operator-candidate-provenance.md) is research evidence, not selection or recovery proof | `<artifact/decision evidence record ID>` | CANDIDATE EVIDENCE ONLY — STOP |
