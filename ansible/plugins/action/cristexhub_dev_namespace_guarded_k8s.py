...
class ActionModule(KubernetesActionModule):
    """Permit only the exact DEV Namespace mutation without task selection."""
    ...
    def run(
        ...
        if self._task.args != _EXPECTED_ARGUMENTS:
            return {
                ...
                "msg": (
                    ...
                    "present-only cristexhub-dev Namespace contract"
                ),
            }
        ...
        if start_at_task or step or tags not in ([], ["all"]) or skip_tags:
            return {
                ...
                "failed": True,
                "msg": (
                    "TASK_SELECTION_GUARD: refusing the CristexHub DEV Namespace "
