...
| KIF-014 | Infisical Cloud provides separate DEV, PROD, and infrastructure scopes/identities; the Kubernetes Operator is live in `shared-services`, and self-hosted Infisical remains deferred. The source-only database Secret seam remains a value-free blocked contract. The separate CristexHub PROD runtime seam is materialized at `/cristexhub/prod/runtime` with independent PROD Auth/Universal Auth names, historical nine-key runtime plus GHCR-pull targets, scoped VAP/bindings, and writer RBAC; historical apply and final idempotence passed at `ok=62 changed=0 failed=0 skipped=3`. The selected ten-key Browserless source update remains NOT RUN/BLOCKED. Keycloak is a separate deployment in `shared-services`; it authenticates and emits groups, Argo RBAC authorizes Argo actions, and Kubernetes RBAC independently constrains controllers; direct Argo OIDC is selected with Dex absent. Residual MongoDB/RabbitMQ/GHCR credential rotation and MongoDB NetworkPolicy acceptance remain open. |
...
| KIF-017-RR | The live DEV successor scope is `reactive_resume_dev_successor` with separate `reactive_resume_dev_runtime` and `reactive_resume_dev_migrator` NOINHERIT roles and `reactive-resume-dev-runtime`/`reactive-resume-dev-migration` custody. Logical data restore excludes roles/ACLs/passwords; those remain separate Infisical/CNPG recovery gates. |
...
| KIF-023 | Future CristexHub-owned publication requires reviewed immutable build inputs, exact commit/digest evidence, SBOM/provenance, collision handling, and recovery. Any future upstream Reactive Resume or platform image selection must use an accepted immutable digest rather than a rebuild or mutable tag; the private DEV runtime digest is selected only as a live immutable DEV input; provenance, vulnerability/recovery trust, promotion, and workloads deploying `latest` remain forbidden. |
