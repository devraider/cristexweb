...
| KIF-K3S-02 | KIF-002, KIF-005, KIF-007, KIF-013–KIF-015, KIF-027, KIF-030 | Read-only k3s datastore/encryption preflight | Check-only fixed commands classify only safe default/exact data-dir evidence, treat unread config as unknown, normalize encryption status, reject stage-only output, protect every datastore/output path component, and emit only sanitized local evidence | PARTIAL / LIVE STOPPED BEFORE COLLECTION — 8 focused contracts, including the required `--become`/`--ask-become-pass` pair and controller-local no-become boundaries, and executable parser fixtures pass; the live read-only wrapper invocation failed at the `ansible_become` gate because `--become` was omitted alongside `--ask-become-pass`; datastore type, encryption status, backup format, key custody, and restore compatibility remain `UNKNOWN — STOP` |
