...
| KIF-RR-01 | KIF-012–KIF-017, KIF-019, KIF-021, KIF-023, KIF-026–KIF-030 | Reactive Resume private-MVP source architecture | Include environment-local Reactive Resume DEV in the private MVP, reserve separate PROD, bind exact OIDC clients and dedicated shared-PostgreSQL scopes, keep Infisical value ownership/private exposure, and distinguish live checkpoint evidence from source reproducibility, image trust, and full acceptance | PASS PRIVATE DEV RUNTIME/ARGO CHECKPOINT — pinned Argo revision `dd7d4cedd902e68266d9713d1dbb8e90f0b529b1` has exactly seven resources including default-deny, private hostname/Traefik route, materialized `reactive-resume-dev-runtime` Secret and DEV CA, shared-realm session/logout cycle, and successor PostgreSQL scope checkpointed; it is `Synced/Healthy`. Current HEAD contains an additional checked-in `networkpolicy-allow-backend.yaml` YAML manifest, which is source-only and is not claimed live, Argo-managed, or applied. This is live checkpoint evidence, not full acceptance or source-reproducibility evidence. Current main includes a dedicated, value-free Infisical DEV CA StaticSecret/VAP/RBAC closure for the two CA projections, but no complete Reactive Resume runtime/migration StaticSecret/VAP/writer source or successor database provisioning wrapper; the live successor SQL objects remain accurate observations but source check/apply/idempotence and ACL/cross-database acceptance are pending. The selected immutable GHCR digests are live DEV inputs only, not image trust/provenance/vulnerability/off-node recovery or promotion acceptance. Hardened schema-2 backup/restore, measured RPO/RTO, TLS renewal, soak, PROD/public promotion, and final handoff remain pending |
| KIF-RR-02 | KIF-005, KIF-013, KIF-015, KIF-021, KIF-026, KIF-030 | Reactive Resume DEV soak and schema-2 receipt correlation offline contract (2026-09-04) | A clearly synthetic, value-free fixture proves the fixed 16 samples at 60-second offsets through a 900-second window, every sample's `exposure_pass`, unique object keys under the exact reviewed prefixes with exact inventory totals, schema-2 restore fields, independent restore-start/completion epoch-derived RTO, backup-duration and RPO epoch correlations, archive hashes, exact receipt keys, matching backup/restore run IDs and source-closure hashes, and `private_residue=none`; it must not be mistaken for live acceptance | PASS SOURCE-ONLY — `PYTHONDONTWRITEBYTECODE=1 /home/paul/projects/cristexweb/.venv/bin/python -m unittest -v tests.test_reactive_resume_dev_acceptance_contract tests.test_reactive_resume_dev_soak_contract tests.test_reactive_resume_dev_backup_contract` passed (fixture and source contracts); YAML parse, changed-role assertions, and negative identity/total/epoch/correlation/window/residue cases passed. No inventory host, Kubernetes/API, provider, Secret, backup, restore, soak, or live receipt was accessed or created. Live 16-sample soak, hardened schema-2 backup/restore, measured RPO/RTO, and scheduler acceptance remain NOT RUN/BLOCKED |
...
- **Reactive Resume DEV:** The private runtime/hostname checkpoint is live, with
  exact seven-object Argo adoption at pinned revision
  `dd7d4cedd902e68266d9713d1dbb8e90f0b529b1`; current HEAD's additional
  `networkpolicy-allow-backend.yaml` remains source-only and is not claimed live,
  Argo-managed, or applied. The pinned revision is `Synced/Healthy`; its private
...
separately from full acceptance: the pinned revision
`dd7d4cedd902e68266d9713d1dbb8e90f0b529b1` has seven Argo manifests including
default-deny, private Traefik route, materialized CA, runtime Secret
`reactive-resume-dev-runtime`, selected DEV digests, and successor PostgreSQL
scope `reactive_resume_dev_successor`. Current HEAD's additional
`networkpolicy-allow-backend.yaml` remains source-only and is not claimed live,
Argo-managed, or applied. Image
