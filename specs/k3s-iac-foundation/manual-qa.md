...
was published, and the Reactive Resume image remains candidate-only with no callback,
application object, or runtime selected. Existing broad-lane database/credential
objects are unaccepted drift.
...
| MQA-15 | KIF-012–KIF-017, KIF-021 | Private Reactive Resume DEV | Digest-pinned DEV instance uses the exact private Keycloak client and its dedicated PostgreSQL scope; cross-environment/database access and public/admin exposure fail closed; backup/restore succeeds | PENDING — no application workload exists. Broad-lane DEV/PROD PostgreSQL CRs and credential Secrets are live but unaccepted; roles retain `INHERIT`, PUBLIC access is broad, no NetworkPolicy selects shared PostgreSQL, and exposed credentials require rotation/revocation. Dedicated Infisical/VAP/RBAC, accepted image/OIDC/storage/database lane, runtime, and recovery remain blocked |
