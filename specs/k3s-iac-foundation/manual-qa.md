...
manual case: PostgreSQL and MongoDB source contracts exist. Read-only evidence now
shows Reactive Resume DEV/PROD PostgreSQL CRs and Infisical-owned Secrets live from
the broad lane, but their provenance, rotation/revocation, storage behavior,
authorization, backup/restore, and runtime acceptance remain pending. The SHA-pinned CI and Reactive Resume
...
application run is not observable with the available unauthenticated API, the private DEV GHCR runtime/migration digests are selected for the live Argo checkpoint,
while provenance/promotion remains unaccepted. The pinned seven-manifest Argo application at revision
`dd7d4cedd902e68266d9713d1dbb8e90f0b529b1`, private route, runtime Secret, and
materialized CA are live. Current HEAD contains the additional checked-in
`networkpolicy-allow-backend.yaml` manifest, which remains source-only and is not
claimed live, Argo-managed, or applied. A separate recorded private-runtime acceptance also
covered the active shared-realm client and private-hostname OIDC/session/logout
flow; the source-only client check did not perform that runtime reconciliation.
Existing predecessor broad-lane database/credential objects remain unaccepted
drift.
...
| MQA-15 | KIF-012–KIF-017, KIF-021 | Private Reactive Resume DEV | Digest-pinned DEV instance uses the exact private Keycloak client and its dedicated PostgreSQL scope; cross-environment/database access and public/admin exposure fail closed; backup/restore succeeds | PENDING — the pinned seven-manifest Argo workload at revision `dd7d4cedd902e68266d9713d1dbb8e90f0b529b1` is `Synced/Healthy` with the selected DEV digests; current HEAD's additional `networkpolicy-allow-backend.yaml` remains source-only and is not claimed live, Argo-managed, or applied. The private Traefik route, `reactive-resume-dev-runtime`, materialized CA, and a separate recorded private-runtime shared-realm OIDC/session/logout acceptance. Full OIDC claim/negative-test and database ACL, image provenance/promotion, data-only backup/restore, TLS renewal, soak, and exposed predecessor credential rotation/revocation remain blocked |
