...
| MQA-15 | KIF-012–KIF-017, KIF-021 | Private Reactive Resume DEV | Digest-pinned DEV instance uses the exact private Keycloak client and its dedicated PostgreSQL scope; cross-environment/database access and public/admin exposure fail closed; backup/restore succeeds | PENDING — checked source-only DEV blocker inventory reserves only the private `cristexhub-dev` object boundary and dedicated shared-PostgreSQL scope; dedicated Infisical/VAP/RBAC, image, OIDC, object storage, Secrets, database, runtime, and recovery remain blocked |
