...
while provenance/promotion remains unaccepted. The seven-manifest Argo application,
private route, runtime Secret, materialized CA, and shared-realm client are live;
existing predecessor broad-lane database/credential objects remain unaccepted drift.
...
| MQA-15 | KIF-012–KIF-017, KIF-021 | Private Reactive Resume DEV | Digest-pinned DEV instance uses the exact private Keycloak client and its dedicated PostgreSQL scope; cross-environment/database access and public/admin exposure fail closed; backup/restore succeeds | PENDING — the private seven-manifest Argo workload is `Synced/Healthy` with the selected DEV digests, private Traefik route, `reactive-resume-dev-runtime`, materialized CA, and successor database scope. Full OIDC/database ACL, image provenance/promotion, data-only backup/restore, TLS renewal, soak, and exposed predecessor credential rotation/revocation remain blocked |
